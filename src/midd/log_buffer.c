...
*/
#ifdef TEST_LOCAL
    // #define REPEAT 20
    // #define VALUE_SEG_SIZE 64*4
    #define REPEAT 1024
    #define VALUE_SEG_SIZE 1024*1024*4
    void* writer_thread(void * args)
    {
        ...
        {
            size_t key_size  = bit_count(count) + 1;
            key_ptr   = (char * )malloc(key_size);
            value_ptr = (char *)malloc(VALUE_SEG_SIZE);
            memset(key_ptr, 0 , key_size);
            memset(value_ptr, 1 , VALUE_SEG_SIZE);

            sprintf(key_ptr, "%d", count);
            // MID_LOG("key: %s, len is %d", key_ptr, strlen(key_ptr));
            *(value_ptr + (VALUE_SEG_SIZE)) = '\0'; 
            *(value_ptr + (VALUE_SEG_SIZE + TAG)) = 1; 
        
            if(!main_node_write_log(key_ptr, value_ptr))
            {
                ERROR_LOG("main_node_write_log fatal error, exit!");
                exit(0);
            }
        }
...
bool main_node_write_log(char * key, char * value)
{
    ...
    MID_LOG("writer_thread: log id is [%d], key is \"%s\", key_len is %d, value_len is %d, total move size is %d", \
                        log->mateData.id, (char*)log + LOG_KEY_OFFSET(*log), KEY_LEN(*log), VALUE_LEN(*log), send_len + value_len);
    ...
    /* 发送其余的元数据信息 */
    for (node_id = server_instance->server_id + 2 ; node_id < node_nums; node_id++)
    {
        int mate_pos;
        RemoteRingbuff* rbuff = remote_buffs_list[node_id];

        while(!check_remote_size(remote_buffs_list[node_id], send_len + value_len))
        {
            MID_LOG("remote node [%d]'s buffer is full, please retry!", node_id);
            sleep(1);
        }
...
void * NIC_thread(void * args)
{
    ...
    for(;;)
    {
        if(!emptyQueue(sending_queue))
        {
            ...
            // free_log(log);
            MID_LOG("NIC_thread has write log [%d]", log->mateData.id);
            count++;
        ...
        else
        {
            ...
            if(test_done(tag_addr))
            {
                ...
                if(value_pos + VALUE_LEN(*log) >= buffer_size)
                {
                    ...
                    MID_LOG("read_log_value: value is cut");
                }
                else
                {
                ...
                read_num++;
                MID_LOG("read_log_value: key is \"%s\", key len is %d, value len is %d", \
