...
{
    size_t wr_pointer;       // 写指针cache
    size_t rd_pointer;       // 读指针cache
    size_t size;                      // 字节大小
}Ringbuff;
...
/* 所以 length = capacity - 1 的时候，q3.h 就认为队列已经满了，所以 q3.h 最大的实际长度只能为 capacity - 1 个元素 */

#define rb_length (rb->wr_pointer - rb->rd_pointer)

//计算数据空间大小
static inline size_t rb_data_size (Ringbuff *rb)    
{
    return ( (rb->wr_pointer - rb->rd_pointer) & (rb->size -1));   
}

//计算空闲空间大小
static inline size_t rb_free_size (Ringbuff *rb)   
{
    return ( rb->size - 1 - rb_data_size(rb));
}

//计算从读指针pos开始的空闲空间大小
static inline size_t rb_count_size (Ringbuff *rb, int pos)   
{
